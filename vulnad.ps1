...
function VulnAD-AddACL {
    ...
    # Apply an Active Directory access rule (ACE) to the destination object
...
function VulnAD-BadAcls {
    # Assign a selection of dangerous ACL rights between groups and objects to simulate insecure delegations
    ...
    for ($i = 1; $i -le (Get-Random -Maximum 25); $i = $i + 1) {
        ...
        if ($null -eq $randomuser -or $null -eq $randomgroup) { continue }
...
function VulnAD-Kerberoasting {
    # Create one intentionally weak service account with an SPN so it can be targeted by Kerberoasting
    ...
    Try {
        New-ADServiceAccount -Name $svc -ServicePrincipalNames "$svc/$spn.$Global:Domain" `
            -AccountPassword (ConvertTo-SecureString $password -AsPlainText -Force) `
            -RestrictToSingleComputer -PassThru
    } Catch {}

    # Create the rest of the service accounts with stronger generated passwords
...
function VulnAD-ASREPRoasting {
    # Create some users that do not require pre-authentication and set weak passwords to simulate AS-REP Roasting targets
    for ($i = 1; $i -le (Get-Random -Maximum 6); $i = $i + 1) {
        $randomuser = (VulnAD-GetRandom -InputList $Global:CreatedUsers)
        if ($null -eq $randomuser) { continue }
        $password = VulnAD-GetRandom -InputList $Global:BadPasswords
        Try {
...
function VulnAD-DnsAdmins {
    # Add several created users to DNSAdmins and also add a mid-level group as nested group to DNSAdmins
    for ($i = 1; $i -le (Get-Random -Maximum 6); $i = $i + 1) {
        $randomuser = (VulnAD-GetRandom -InputList $Global:CreatedUsers)
        if ($null -ne $randomuser) {
            Try { Add-ADGroupMember -Identity "DnsAdmins" -Members $randomuser } Catch {}
            Write-Info "DnsAdmins : $randomuser"
        }
    }
    $randomg = (VulnAD-GetRandom -InputList $Global:MidGroups)
    if ($null -ne $randomg) {
        Try { Add-ADGroupMember -Identity "DnsAdmins" -Members $randomg } Catch {}
        Write-Info "DnsAdmins Nested Group : $randomg"
    }
}

function VulnAD-PwdInObjectDescription {
    # Put generated passwords into account description fields for a subset of users
    for ($i = 1; $i -le (Get-Random -Maximum 6); $i = $i + 1) {
        $randomuser = (VulnAD-GetRandom -InputList $Global:CreatedUsers)
        if ($null -eq $randomuser) { continue }
        $password = ([System.Web.Security.Membership]::GeneratePassword(12,2))
...
function VulnAD-DefaultPassword {
    # Assign a default known password to some users (and require change at next logon)
...
function VulnAD-PasswordSpraying {
    # Give multiple users the same password to simulate password spraying vulnerabilities
...
function VulnAD-DCSync {
    # Grant selected users replication-related rights (GetChanges/GetChangesAll/Replication-Get-Changes) on the domain root
    for ($i = 1; $i -le (Get-Random -Maximum 6); $i = $i + 1) {
        ...
        $ADObject.psbase.Get_ObjectSecurity().AddAccessRule($ACEGetChangesAll)

        $objectGuidReplication = New-Object Guid 89e95b76-444d-4c62-991a-0facbeda640c
        $ACEReplication = New-Object DirectoryServices.ActiveDirectoryAccessRule($sid,'ExtendedRight','Allow',$objectGuidReplication)
        $ADObject.psbase.Get_ObjectSecurity().AddAccessRule($ACEReplication)

        $ADObject.psbase.CommitChanges()

        Try { Set-ADUser $randomuser -Description "Replication Account" } Catch {}
        Write-Info "Giving DCSync to : $randomuser"
    }
}

function VulnAD-DisableSMBSigning {
    # Disable SMB client signing to simulate a poorly configured environment
    Try { Set-SmbClientConfiguration -RequireSecuritySignature 0 -EnableSecuritySignature 0 -Confirm:$false -Force } Catch {}
}
...
function Invoke-VulnAD {
    ...
    # Apply a weak domain password policy to simulate insecure settings
    Try {
        Set-ADDefaultDomainPasswordPolicy -Identity $Global:Domain `
            -LockoutDuration 00:01:00 -LockoutObservationWindow 00:01:00 `
            -ComplexityEnabled $false -ReversibleEncryptionEnabled $false `
            -MinPasswordLength 4 -MinPasswordAge 00:00:00 -MaxPasswordAge 365.00:00:00
    } Catch {}
